                    ...
                    if (obj.sprite && obj.sprite.pictures) {
                        obj.sprite.pictures.forEach((picture, picIndex) => {
                            if (picture.filename && !picture.fileurl) {
                                // 8070번 서버 상대경로로 설정: /temp/xx/xx/image/filename.ext
                                ...
                                console.log(`🔄 기존 경로 수정 [${index}-${picIndex}]:`, picture.fileurl);
                            }
                        });
                    }
                    
                    // 오브젝트 썸네일 경로 수정
                    if (obj.thumbnail && typeof obj.thumbnail === 'string' && obj.thumbnail.startsWith('temp/')) {
