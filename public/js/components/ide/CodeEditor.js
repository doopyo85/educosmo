...
class CodeEditor {
  constructor(options = {}) {
    ...
    // 🔥 폰트 크기 관리 - 향상된 버전
    this.fontState = {
      currentFontSize: parseInt(localStorage.getItem('ide-font-size') || '20'),
      minFontSize: 10,
      maxFontSize: 32
    };
    ...
    try {
      ...
      if (data.success && data.code) {
        ...
        if (this.state.editor) {
          ...
          // 🔥 보일러플레이트 체크 및 처리
          if (this.checkBoilerplate(data.code)) {
            this.state.editor.setValue('', -1);
            this.showPlaceholder(data.code);
          } else {
            this.state.editor.setValue(data.code);
            this.state.editor.clearSelection();
            this.state.editor.gotoLine(1);
            this.hidePlaceholder();
            this.placeholderState.text = ''; // Clear placeholder text if real code loaded
          }
   ...
   * 🔥 보일러플레이트 코드 확인
   */
  checkBoilerplate(code) {
    if (!code) return false;
    const lines = code.split('\n');
    // 10줄 미만이고 모든 줄이 주석(#)으로 시작하거나 비어있으면 보일러플레이트로 간주
  ...
  setupPlaceholder() {
    ...
    placeholder.className = 'ace_placeholder';
    this.state.editor.renderer.scroller.appendChild(placeholder);
    this.placeholderState.element = placeholder;

    // Event Listeners
    this.state.editor.on('input', () => this.updatePlaceholderVisibility());
    this.state.editor.on('focus', () => this.hidePlaceholder());
    this.state.editor.on('blur', () => this.showPlaceholderIfEmpty());

    // Update position on render/scroll
    this.state.editor.renderer.on('afterRender', () => this.updatePlaceholderPosition());
  }

  updatePlaceholderPosition() {
    if (!this.placeholderState.element || !this.state.editor) return;

    // const gutterWidth = this.state.editor.renderer.$gutterLayer.element.offsetWidth;
    // this.placeholderState.element.style.left = gutterWidth + 'px'; 
    // ACE scroller handles relative position, but we might need margin
    // Actually, appending to scroller moves it with content.
    // We want it fixed? No, it should scroll if long? 
    // But boilerplate is short.
    // Appending to scroller is fine.
    // Padding logic handled in CSS.
  }

  showPlaceholder(text) {
    if (!this.placeholderState.element) return;
    this.placeholderState.text = text;
    this.placeholderState.element.textContent = text;
    this.placeholderState.element.style.display = 'block';
    this.placeholderState.isActive = true;
  }

  hidePlaceholder() {
    if (!this.placeholderState.element) return;
    this.placeholderState.element.style.display = 'none';
    this.placeholderState.isActive = false;
  }

  showPlaceholderIfEmpty() {
    const content = this.state.editor.getValue();
    if (!content || !content.trim()) {
      if (this.placeholderState.text) {
        this.showPlaceholder(this.placeholderState.text);
      }
    }
  }
  ...
  updatePlaceholderVisibility() {
    const content = this.state.editor.getValue();
    if (!content || !content.trim()) {
      // Do nothing on input empty? 
      // Usually hidden on input.
      // Only show on blur if empty.
    } else {
      this.hidePlaceholder();
    }
  }
  ...
  setDefaultTemplate(examName = 'Unknown', problemNumber = '01') {
    ...
    if (this.state.editor) {
      if (this.checkBoilerplate(defaultCode)) {
        this.state.editor.setValue('');
        this.showPlaceholder(defaultCode);
      } else {
  ...
  restoreExampleCode() {
    ...
    if (this.state.editor) {
      // 🔥 보일러플레이트 체크
      if (this.checkBoilerplate(this.exampleCodeState.originalCode)) {
        this.state.editor.setValue('', -1);
        this.showPlaceholder(this.exampleCodeState.originalCode);
        document.activeElement.blur(); // Focus out to show placeholder
      } else {
