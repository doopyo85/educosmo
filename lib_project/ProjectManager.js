...
class ProjectManager {
    ...
    async saveProject(options) {
        ...
        try {
            ...
            // 4. S3 키 생성
            const s3Key = this.generateS3Key(platform, userId, projectName, saveType, missionId);
            ...
            const submissionId = await this.saveToDatabase({
                ...
                metadata: {
                    ...metadata,
                    missionId, // 🔥 Save missionId in metadata
                    analysis
                },
    ...
    /**
     * 🔥 S3 키 생성 (통일된 규칙)
     * - 기본: users/{userID}/{platform}/{saveType}/{projectName}_{timestamp}.{ext}
     * - 미션: {platform}/{userID}/{missionId}/{projectName}_{timestamp}.{ext} (제출용)
     */
    generateS3Key(platform, userId, projectName, saveType, missionId) {
        ...
        if (missionId) {
            // 🔥 과제 제출용 경로 (s3://codingnplay-learning-data/scratch/user_id/mission_id/...)
            // saveType은 경로에 포함하지 않음 (보통 final/submit 일 것임)
            return `${ platform } /${userId}/${ missionId }/${sanitized}_${timestamp}.${extension}`;
        } else {
    // 🔥 기존 개인 프로젝트 경로
