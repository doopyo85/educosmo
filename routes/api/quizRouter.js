  ...
  try {
    ...
    if (problemNumber) {
      ...
      const quizData = {
        ...
        exampleFileUrl: problem[5] || '',         // F: 예제파일URL (기본 코드 파일)
        answerFileUrl: problem[6] || '',          // G: 정답파일URL
        explanationFileUrl: problem[7] || '',    // H: 해설파일URL
        answerType: problem[8] || 'number',       // I: answerType
        correctAnswer: problem[9] || '',          // J: 정답 (JSON 형태)
        testCases: problem[10] || '[]',           // K: 인풋/아웃풋 (테스트케이스)
        difficulty: problem[11] || '1',           // L: 난이도
        questionType: problem[12] || '객관식',     // M: 객관식/주관식
        tags: problem[13] || ''                   // N: Tags
      };
    ...
    const quizDataList = matchingProblems.map(problem => ({
      ...
      concept: problem[3] || '',
      fileUrl: problem[4] || '',
      exampleFileUrl: problem[5] || '',
      answerFileUrl: problem[6] || '',
      explanationFileUrl: problem[7] || '',
      answerType: problem[8] || 'number',
      correctAnswer: problem[9] || '',
      testCases: problem[10] || '[]',
      difficulty: problem[11] || '1',
      questionType: problem[12] || '객관식',
      tags: problem[13] || ''
    }));
...
router.post('/submit-answer', async (req, res) => {
  try {
    ...
    const correctAnswerData = problem[9] || '';      // J: 정답
    const questionType = problem[12] || '객관식';     // M: 객관식/주관식
    const problemTestCases = problem[10] || '[]';    // K: 인풋/아웃풋
    const actualAnswerType = problem[8] || 'number'; // I: answerType
